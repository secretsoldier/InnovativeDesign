...
public class NPieChart { // Copied and altered from SortingGG.java
    ...
    private static Map<String, NPieChartObj> RootToMap(File root){ // Converts the directory into data that the chart can process
        ...
        for (File file : files) // Loop through the list of files in the directory "root"
        {
            ...
            NPieChartObj currentObj = map.get(extension); // Gets the extension object data from the extension given
            ...
            if (file.length() > 0) { // If file size is larger than 0
                if (file.isFile() && map.containsKey(extension)) { // If file is a file and is present in the extension map
                    ...
                    NPieChartObj newExtensionData = new NPieChartObj(extension, file.length(), new Color((int)(Math.random() * 256), (int)(Math.random() * 256), (int)(Math.random() * 256)));
    ...
    public static NPieChartC createPieChartJComponent(File root){
        return new NPieChartC(RootToMap(root)); // Returns the Pie Chart Component from a File parameter
    }
